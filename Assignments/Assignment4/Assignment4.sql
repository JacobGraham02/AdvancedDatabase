...
Functions must return a single value, where a stored procedure can return 0, 1, or multiple valuess
...
The function needs one input EMPLOYEE NUMBER.   With that number SELECT the EMPTIME for that employee into a variable FROM the EMPROJACT table.   
Then Create a new select that will Multiply that EMPTIME value by the COMM column, then  add the SALARY and BONUS columns from the EMPLOYEE table for that EMPLOYEE to get a return a total cost for that employee.
...
Create a user-defined function (F_GET_LET_NUMB) to do the following:
	It will accept 2 inputs DATA_STRING and L_OR_N.    The DATA_STRING will be a varchar with letters and numbers.   The value of L_OR_N will be NUMB or  LETT.    
