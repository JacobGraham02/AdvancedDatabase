...
-- );
CREATE TABLE PRODUCT_HST AS (SELECT PID FROM PRODUCT)
ALTER TABLE PRODUCT_HST 
ADD COLUMN PRICE_HST DOUBLE,
ADD COLUMN PROMOPRICE_HST DOUBLE;
...
CREATE TRIGGER INSERT_PRODUCT 
	AFTER INSERT ON PRODUCT FOR EACH ROW
		BEGIN
			SET HST = 1.13;
			INSERT INTO PRODUCT_HST (PRICE_HST, PROMO_PRICE_HST) 
			(SELECT PRICE = NEW.PRICE * HST, PROMOPRICE = NEW.PROMOPRICE * HST); 
		END
	//
